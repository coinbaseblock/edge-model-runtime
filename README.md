...
- 🖥️ **Agents in the browser** — optional [ttyd-based terminal](./docs/AI-CODING-SETUP.md#option-d--agents-in-the-browser-no-terminal-needed) launches OpenCode or Claude Code from a menu at `http://localhost:7681` — no SSH client required
- 🪪 **Claude Code with local brain** — point the `claude` CLI at LiteLLM (Option E) to drive [Claude's TUI with a local Ollama model](./docs/AI-CODING-SETUP.md#option-e--claude-code-tui-driving-a-local-model) — same UX as cloud Claude Code, $0 per token
